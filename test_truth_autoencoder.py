...
def run_inference(model, loader, mean_np, std_np):
    ...
    latent = np.concatenate(latent_rows, axis=0)
    metrics = {
        ...
        "latent_l2_mean": float(np.mean(np.linalg.norm(latent, axis=1))),
    }
    return pred, gt, latent, metrics


def plot_window_trajectories(pred, gt, out_dir, max_examples=6):
    if pred.shape[0] == 0:
        return
    ...
    for ax, sample_idx in zip(axes, idx):
        gt_path = truth_to_path(gt[sample_idx : sample_idx + 1])[0]
        pred_path = truth_to_path(pred[sample_idx : sample_idx + 1])[0]
    ...
    fig.suptitle(f"Truth reconstruction windows: {dataset_name} / {truth_mode}")
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, "test_window_trajectories.png"), bbox_inches="tight")
    plt.close(fig)


def plot_window_series(pred, gt, out_dir, sample_idx=0):
    if pred.shape[0] == 0:
        return
    sample_idx = min(max(sample_idx, 0), pred.shape[0] - 1)
    t = np.arange(pred.shape[1])
    labels = ["x", "y", "z"]
    fig, axes = plt.subplots(3, 1, figsize=(10, 8), dpi=150, sharex=True)
    for axis_idx in range(3):
        axes[axis_idx].plot(t, gt[sample_idx, :, axis_idx], color="black", linewidth=1.5, label="GT")
        axes[axis_idx].plot(t, pred[sample_idx, :, axis_idx], color="red", linewidth=1.1, alpha=0.85, label="Recon")
        axes[axis_idx].set_ylabel(labels[axis_idx])
        axes[axis_idx].grid(True, linestyle=":", alpha=0.5)
    axes[0].legend(loc="best", fontsize=8)
    axes[-1].set_xlabel("Frame In Window")
    fig.suptitle(f"Truth series reconstruction: window {sample_idx}")
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, "test_window_series.png"), bbox_inches="tight")
    plt.close(fig)
...
def main():
    ...
    print(
        ...
        f"path_len_mae={metrics['path_len_mae']:.4f}m latent_l2_mean={metrics['latent_l2_mean']:.4f}"
    )

    plot_window_trajectories(pred, gt, output_dir)
    plot_window_series(pred, gt, output_dir, sample_idx=0)
